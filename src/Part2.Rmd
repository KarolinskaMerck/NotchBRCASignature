...
## First: Get full gene universe

id_merck_tcga_rna_wtstmm_gexp <- "merck_tcga_rna_wtstmm_gexp_21q2_prod_kreis_2"

TCGA_gene_universe = api$conn %>%
  tbl(id_merck_tcga_rna_wtstmm_gexp) %>% 
  dplyr::filter(id_tcga_cohort == "BRCA",
                sample_status == "primary tumor") %>%
  dplyr::select(id_gene_symbol) %>%
  dplyr::collect() %>% 
  dplyr::distinct()

metabric_gene_universe <- api$conn %>%
  tbl(id_metabric) %>% 
  dplyr::select(id_gene_symbol) %>% 
  collect() %>% 
  dplyr::distinct()

Oslo2_gene_universe <- api$conn %>%
  tbl(id_oslo2) %>% 
  dplyr::select(id_gene_symbol) %>% 
  collect() %>% 
  dplyr::distinct()

gene_universe_3_datasets <- unique(c(TCGA_gene_universe$id_gene_symbol, 
                              metabric_gene_universe$id_gene_symbol,
                              Oslo2_gene_universe$id_gene_symbol))
...
# Now use the gene universe that we filtered for in the TNBC BRCA data set
