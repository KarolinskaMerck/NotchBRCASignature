...
subtitle: "Part 1b - Training Data: Basal-like treated BRCA Cell Lines, 8h treatment"
...
output: 
  html_document:
    ...
    toc_float:
      collapsed: FALSE
      smooth_scroll: FALSE
bibliography: "`r here::here('src', 'part1b.bib')`"
link-citations: true
...
knitr::opts_chunk$set(
	...
	fig.align = "center",
#	fig.path = "../Figures/Part1b/",           # if you want all the figures directly written into a seperate folder
	message = FALSE,
	warning = FALSE,
...
knitr::write_bib(file = '../src/part1b.bib')
...
## define default theme and colors
...
Here, differential expression analysis has been performed only for the 8h time point.  

**Conclusion**  

<div class = "box">

Differentially expression analysis of the training data set (8h only) of the six basal-like cell lines resulted in lists of differentially expressed genes.  
...
ddsSE_8h_preparation_8h <- ddsSE_preparation[,ddsSE_preparation$condition_time == "8h"]
design(ddsSE_8h_preparation_8h) <- ~ condition_lib_prep + condition_cell_line + condition_treatment

ddsSE_8h <- DESeq(ddsSE_8h_preparation_8h)
...
```{r 04a-DESEQ2-QC-meanSD-8h, include=FALSE}

# this gives log2(n + 1)
ntd <- normTransform(ddsSE_8h)
p <- meanSdPlot(assay(ddsSE_8h), 
                plot = FALSE)
```

```{r 04b-DESEQ2-QC-meanSD-8h}
p$gg +
  ggtitle("Diagnostic Plot: Standard deviations vs row means [log2(n+1)] (8h)")
```

```{r 04c-DESEQ2-QC-meanSD-8h, include=FALSE}
# variance stabilized
vsd <- vst(ddsSE_8h, blind=FALSE)
p <- meanSdPlot(assay(vsd), 
                plot = FALSE)
```

```{r 04d-DESEQ2-QC-meanSD-8h}
p$gg +
  ggtitle("Diagnostic Plot: Standard deviations vs row means [variance stabilized (vst)] (8h)")
...
meta.all_summary_8h <- meta.all_summary %>% 
  dplyr::filter(condition_time == "8h")

### --- Sample Quality

ggplot(meta.all_summary_8h,
       aes(x=condition_cell_line,
           y=meta_rin)) +
  geom_point() +
  ggtitle("RIN values by cell line (8h only)") +
  labs(y = "RIN values",
       x = "Cell Lines")
...
ggplot(meta.all_summary_8h,
       aes(x=condition_cell_line,
           y=meta_lib_conc)) +
  geom_point() +
  ggtitle("Library Concentrations by cell line (8h only)") +
...
ggplot(meta.all_summary_8h,
       aes(x=condition_cell_line,
           y=meta_lib_size_bp)) +
  geom_point() +
  ggtitle("Average library size by cell line (8h only)") +
  labs(y = "average library size [bp]",
       x = "Cell Lines")
```

```{r 5a-PCA-correlations-8h, echo=TRUE, message=FALSE, warning=FALSE, fig.width= 12, fig.height=4}
...
```{r 05b-PCA-screeplot-8h, echo=TRUE, message=FALSE, warning=FALSE, fig.width= 4, fig.height=3.3}
...
screeplot(p,
  ...
  labs(title = "Screeplot - 8h")

```

### PCA

```{r 05c-PCA-8h, echo=TRUE, message=FALSE, warning=FALSE, fig.width= 6, fig.height=5}

biplot(p,
       title = "PC1 vs PC2",
       subtitle = "Cell Line and treatment condition (8h)",
       lab = NA,
...
biplot(p,
       title = "PC1 vs PC2",
       subtitle = "Library Prep Batch and treatment condition (8h)",
       lab = NA,
...
biplot(p,
       ...
       y = "PC13",
       title = "PC5 vs PC13",
       subtitle = "Treatment and Cell Lines (8h)",
       lab = NA,
       colby = 'condition_cell_line',
       shape = "Treatment",
       colkey = as.character(merck_colors),
...
```{r 05d-Umap-8h, echo=TRUE, message=FALSE, warning=FALSE, fig.width= 6, fig.height=5}
...
ggscatter(z_umap_df,
          ...
          title = "UMAP Dimensionality Reduction (8h)") + 
...
ggscatter(z_tumap_df,
          ...
          legend = "right",
          palette = as.character(merck_colors),
          title = "t-UMAP Dimensionality Reduction (8h)") +
...
```{r 06-hm-top-variant-genes-8h, fig.width=8, fig.height=8, dev = "png"}

ntop <- order(matrixStats::rowVars(assay(vsd)), decreasing = TRUE)[1:1000]
cpm_top <- counts(ddsSE_8h,normalized=TRUE)[ntop,]
...
ht <- Heatmap(cpm_top,
        column_title = "Top 1000 variant genes in the breast cancer cell lines (8h)",
        cluster_rows = TRUE,
...
```{r 07-Diagnostic-Plots-8h, fig.width=8, fig.height=8}

qnt.mat_tpm_agg_log2_8h <- qnt.mat_tpm_agg_log2 %>% 
  dplyr::filter(condition_time == "8h")

############################# Diagnostic Plots ############################# 

diag_plot <- function(gene) {
  plotdata <- qnt.mat_tpm_agg_log2_8h %>%
  filter(symbol == !!gene) %>%
  dplyr::mutate(condition_treatment = dplyr::recode_factor(condition_treatment,
                                                           `DAPT`      = 'Notch off',
                                                           `FC`   = 'ground state',
                                                           `Jagged1`     = 'Notch on'),
                condition_time = factor(condition_time, levels = c("8h")))
...
## Notch Expression Heatmap 8h

```{r 08b-notch-expression-hm-8h, fig.height=2.5, fig.width=4}

notch_expression <- qnt.mat_tpm_agg_log2 %>%
  filter(condition_time == "8h") %>%
...
## Endocrine receptor status of cell lines - 8h

```{r 08d-receptors-expression-hm-8h, fig.height=3.5, fig.width=4}
...
```{r 08f-receptors-expression-hm-8h, fig.height=5, fig.width=6}

receptor_expression <- qnt.mat_tpm_agg_log2_8h %>%
  filter(condition_treatment == "FC") %>%
  filter(symbol %in% c("ERBB2","ERBB3","ESR1","PGR","AR")) %>%
...
notch_expression <- qnt.mat_tpm_agg_log2_8h %>%
  filter(condition_treatment == "FC") %>%
  filter(symbol %in% c("NOTCH1", "NOTCH2", "NOTCH3", "NOTCH4")) %>%
  dplyr::select(gene_symbol = symbol, quant_log2tpm, condition_cell_line, condition_subtype, condition_time) %>%
  arrange(gene_symbol, condition_subtype, condition_cell_line, desc(condition_time))
...
```{r 09a-DE-FC-DAPT-8h, fig.width=4, fig.height=4, dev = "png"}

res_DAPT_8h <- results(ddsSE_8h, contrast=c("condition_treatment","DAPT","FC"), alpha = 0.05)

hist(res_DAPT_8h$pvalue)

#We can order our results table by the smallest p value:

res_DAPTOrdered <- res_DAPT_8h[order(res_DAPT_8h$padj),]
#We can summarize some basic tallies using the summary function.

summary(res_DAPT_8h, alpha = 0.05)

plotMA(res_DAPT_8h, ylim=c(-2,2), cex=.2, alpha = 0.05)
abline(h=c(-0.5,0.5), col="dodgerblue", lwd=2)
title(main = "MA-plot FC vs. DAPT - 8h\nlog2 fold changes")

printDF_DAPT <- res_DAPT_8h %>%
  as.data.frame() %>%
  dplyr::filter(!is.na(padj),
                padj < 0.05) %>%
  rownames_to_column("ensembl") %>%
  dplyr::mutate(ensembl = str_remove(ensembl, "\\.[0-9]+$")) %>%
  dplyr::left_join(rowData(ddsSE_8h) %>%
                     as_tibble() %>%
...
```{r 09b-DE-FC-DAPT-vulcano-8h, fig.width=4, fig.height=4}

# resultsNames(ddsSE_8h)

resLFC <- lfcShrink(ddsSE_8h, contrast=c("condition_treatment","DAPT","FC"), type="ashr")
...
ggplot(filtTab %>%
  ...
  labs(x = "log2 fold Change",
       y = "-log10(p adj)",
       title = "Notch Off (DAPT) vs. Control (FC) - 8h")

```

### Jagged1 vs FC - 8h

```{r 10a-DE-FC-Jagged1-8h, fig.width=4, fig.height=4, dev = "png"}

res_Jagged1_8h <- results(ddsSE_8h, contrast=c("condition_treatment","Jagged1","FC"))

hist(res_Jagged1_8h$pvalue)
...
summary(res_Jagged1_8h, alpha = 0.05)

plotMA(res_Jagged1_8h, ylim=c(-2,2), cex=.2, alpha = 0.05)
abline(h=c(-0.5,0.5), col="dodgerblue", lwd=2)
title(main = "MA-plot Jagged1 vs. FC - 8h\nlog2 fold changes")

printDF_Jagged1 <- res_Jagged1_8h %>%
  as.data.frame() %>%
  dplyr::filter(!is.na(padj),
                padj < 0.05) %>%
  rownames_to_column("ensembl") %>%
...
```{r 10b-DE-FC-Jagged1-vulcano-8h, fig.width=4, fig.height=4}
# 
# resultsNames(ddsSE_8h)

resLFC <- lfcShrink(ddsSE_8h, contrast=c("condition_treatment","Jagged1","FC"), type="ashr")
...
```{r 11a-DE-Jagged1-DAPT-vulcano-8h, fig.width=4, fig.height=4, dev = "png"}

res_Jagged1_DAPT_8h <- results(ddsSE_8h, contrast=c("condition_treatment","Jagged1","DAPT"))

hist(res_Jagged1_DAPT_8h$pvalue)

#We can order our results table by the smallest p value:

res_Jagged1_DAPTOrdered <- res_Jagged1_DAPT_8h[order(res_Jagged1_DAPT_8h$padj),]
#We can summarize some basic tallies using the summary function.

summary(res_Jagged1_DAPT_8h, alpha = 0.05)

plotMA(res_Jagged1_DAPT_8h, ylim=c(-2,2), cex=.2, alpha = 0.05)
abline(h=c(-0.5,0.5), col="dodgerblue", lwd=2)
title(main = "MA-plot Jagged1 vs. DAPT - 8h\nlog2 fold changes")

printDF_Jagged1_DAPT <- res_Jagged1_DAPT_8h %>%
  as.data.frame() %>%
  dplyr::filter(!is.na(padj),
                padj < 0.05) %>%
  rownames_to_column("ensembl") %>%
...
```{r 11b-DE-Jagged1-DAPT-vulcano-8h, fig.width=4, fig.height=4}

# resultsNames(ddsSE_8h)

resLFC <- lfcShrink(ddsSE_8h, contrast=c("condition_treatment","Jagged1","DAPT"), type="ashr")
...
save(ddsSE_8h,
     ...
     res_Jagged1_8h,
     res_DAPT_8h,
     file = "../RData/Part1b.RData")
```

# Discussion of the Results and Recommendations

Differentially expression analysis of the training data set (8h) of the six basal-like cell lines resulted in lists of differentially expressed genes.  
